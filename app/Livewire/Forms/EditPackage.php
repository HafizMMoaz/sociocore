...
class EditPackage extends Component
{
    ...
    public $flutterwaveLifetimePlanId;
    ...
    public function submitForm()
    {
        ...
        $previousFeatures = json_decode($this->package->additional_features, true) ?? [];
        $wasAcceptMaintenanceEnabled = in_array('Accept Maintenance Payment', $previousFeatures);
        ...
        $this->package->societies->each(function ($society) {
            clearSocietyModulesCache($society->id);
        });

        $isAcceptMaintenanceDisabled = !in_array('Accept Maintenance Payment', $this->selectedFeatures);

        if ($wasAcceptMaintenanceEnabled && $isAcceptMaintenanceDisabled) {
            PaymentGatewayCredential::first()->update([
