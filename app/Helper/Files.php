...
class Files
{
    ...
    private static function processImage($uploadedFile, $dir, $newName, $width = null, $height = 800)
    {
        ...
        if ($width && $height && !in_array($fileExt, ['svg', 'webp'])) {
            Image::make($tempPath)
                ->resize($width, $height, function ($constraint) {
                    $constraint->aspectRatio();
                    $constraint->upsize();
    ...
    public static function fileStore($file, $folder, $generateNewName = '', $uploaded = false, $societyId = null)
    {
        ...
        $fileStorage->society_id = $societyId;
