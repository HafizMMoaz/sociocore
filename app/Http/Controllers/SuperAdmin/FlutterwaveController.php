...
class FlutterwaveController extends Controller
{
    ...
    public function paymentCallback(Request $request)
    {
        ...
        if (!$transactionRef) {
            return redirect()->route('dashboard')->with([
                'flash.banner' => 'Transaction reference is missing.',
                'flash.bannerStyle' => 'danger'
            ]);
        }
        ...
        if (!$societyPayment) {
            return redirect()->route('dashboard')->with([
                'flash.banner' => 'Invalid transaction reference.',
                'flash.bannerStyle' => 'danger'
            ]);
        }
        ...
        try {
            ...
            if (!isset($responseData['status']) || !isset($responseData['data'])) {
                return redirect()->route('dashboard')->with([
                    'flash.banner' => 'Payment verification failed.',
                    'flash.bannerStyle' => 'danger'
                ]);
            }
            ...
            if ($paymentSuccess) {
                ...
                $subscription->flutterwave_id = $responseData['data']['id'] ?? null;
                $subscription->flutterwave_payment_ref = $transactionRef;
                $subscription->flutterwave_status = $responseData['data']['status'] ?? null;
                $subscription->flutterwave_customer_id = $responseData['data']['customer']['id'] ?? null;
                $subscription->subscription_id = $responseData['data']['id'] ?? null;
                ...
                if (!$package) {
                    return redirect()->route('dashboard')->with([
                        'flash.banner' => 'Package not found.',
                        'flash.bannerStyle' => 'danger'
                    ]);
                }
                ...
                return redirect()->route('dashboard')->with([
                    'flash.banner' => __('Payment verification failed.'),
                    'flash.bannerStyle' => 'danger'
                ]);
            }
        } catch (\Exception $e) {
