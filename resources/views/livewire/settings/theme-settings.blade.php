<div
    ...
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-xl font-semibold dark:text-white">@lang('modules.settings.themeSettings')</h3>
    </div>

    <form wire:submit.prevent="submitForm" x-data="{ photoName: null, photoPreview: null }">
        <div class="space-y-6">
            {{-- Logo Upload Section --}}
            <div
                ...
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            {{ __('modules.settings.logoDescription') }}</p>
                    </div>
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <div class="flex-shrink-0">
                        <div class="relative h-24 w-24">
                            ...
                            <div x-show="!photoPreview"
                                ...
                                <img src="{{ society()->logo_url }}" alt="{{ society()->name }}"
                    ...
                    <div class="flex-grow space-y-3">
                        <input type="file" id="photo" class="hidden"
                            accept="image/png, image/gif, image/jpeg, image/webp, image/svg+xml" wire:model="photo"
                            x-ref="photo"
                            x-on:change="
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                @lang('saas.showLogoTextDescription')
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    @foreach (['upload_fav_icon_android_chrome_192', 'upload_fav_icon_android_chrome_512', 'upload_fav_icon_apple_touch_icon'] as $index => $name)
                        <div
                            class="p-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                            <div class="flex items-center space-x-4">
                                <div class="flex-shrink-0">
                                    <div id="filePreview{{ $name }}"
                                        class="h-12 w-12 rounded-lg bg-gray-50 dark:bg-gray-700 flex items-center justify-center overflow-hidden"
                                        style="background-image: url('{{ ${$name} ? ${$name}->temporaryUrl() : society()->{$name."_url"} }}'); background-size: contain; background-repeat: no-repeat;">
                                    </div>
                                </div>
                    ...
                    @foreach (['upload_favicon_16', 'upload_favicon_32', 'favicon'] as $index => $name)
                        <div
                            class="p-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                            <div class="flex items-center space-x-4">
                                <div class="flex-shrink-0">
                    ...
                    <!-- Add Web Manifest input field -->
                    <div
                        class="p-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                        <div class="flex items-center space-x-4">
                            <div class="flex-shrink-0">
                                <div id="filePreview{{ $name }}"
                                    class="h-8 w-8 rounded bg-gray-50 dark:bg-gray-700 flex items-center justify-center overflow-hidden"
                                    style="background-image: url('{{ asset('img/jsonicon.png') }}'); background-size: contain; background-repeat: no-repeat;">
                                </div>
                            </div>
                            <div class="flex-grow space-y-2">
                                <input type="file" id="webmanifest" class="hidden"
                                    accept="application/manifest+json" wire:model="webmanifest"
                                    x-ref="webmanifest"
                                    x-on:change="
            const reader = new FileReader();
            reader.onload = (e) => {
                document.getElementById('filePreviewWebManifest').style.backgroundImage = 'url(' + e.target.result + ')';
            };
            reader.readAsDataURL($refs.webmanifest.files[0]);
        " />
                                <div class="flex flex-wrap gap-2">
                                    <x-secondary-button type="button"
                                        x-on:click.prevent="$refs.webmanifest.click()"
                                        class="flex items-center text-xs">
                                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        {{ __('saas.siteWebManifest') }}
                                    </x-secondary-button>
                                </div>
                                <x-input-error for="webmanifest" class="mt-2" />
                            </div>
                        </div>
                    </div>
            ...
            <div class="flex items-center justify-end space-x-3">
                <x-button :disabled="$errors->any()" wire:loading.attr="disabled" wire:target="photo"
                    class="min-w-[100px]">
                    @lang('app.save')
                </x-button>
            </div>
