<div class="px-4 pt-2 xl:grid-cols-3 xl:gap-4 bg-white dark:bg-gray-900">
    ...
    <div class="flex items-center justify-end">
        @if($showActions)
            <x-dropdown dropdownClasses="z-50">
                ...
                <x-slot name="content">
                    <x-dropdown-link href="javascript:;" wire:click="showSelectedDeleteTenant">
                ...
                <div class="overflow-hidden shadow">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700" wire:key='user-list-{{ microtime() }}'>
                            ...
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700" wire:key='user-{{ $tenant->id . rand(1111, 9999) . microtime() }}' wire:loading.class.delay='opacity-10'>
                                @if(user_can('Delete Tenant'))
                                    <td class="p-4">
                                        <input type="checkbox" wire:model.live="selected" value="{{ $tenant->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                    </td>
                                @endif
                                <td class="p-4 text-base font-semibold text-gray-900 whitespace-nowrap dark:text-white">
                                    <a href="{{ route('tenants.show', $tenant->id) }}"
                                        ...
                                        <x-avatar-image :src="$tenant->user->profile_photo_url" :alt="$tenant->user->name" :name="$tenant->user->name"/>
                                    </a>
                                </td>
                                ...
                                <td class="p-4 space-x-2 whitespace-nowrap text-right">
                                    @if(user_can('Update Tenant') || $tenant->user_id == user()->id)
                                        <x-secondary-button wire:click="showEditTenant({{ $tenant->id }})" wire:key="tenant-edit-{{ $tenant->id . microtime() }}">
                                            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    @if(user_can('Delete Tenant'))
                                        <x-danger-button  wire:click="showDeleteTenant({{ $tenant->id }})"  wire:key='tenant-del-{{ $tenant->id . microtime() }}'>
                            ...
                            @empty
                                <x-no-results :message="__('messages.noTenantFound')" />
                            @endforelse
    ...
    <div wire:key='tenant-table-paginate-{{ microtime() }}'
    ...
    <x-right-modal wire:model.live="showEditTenantModal">
        <x-slot name="title">
            {{ __("modules.tenant.editTenantDetails") }}
        </x-slot>

        <x-slot name="content">
            @if ($seletedEditTenant)
            @livewire('forms.editTenant', ['tenant' => $seletedEditTenant], key(str()->random(50)))
            @endif
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$set('showEditTenantModal', false)" wire:loading.attr="disabled">
                {{ __('Close') }}
            </x-secondary-button>
        </x-slot>
    </x-right-modal>

    <x-confirmation-modal wire:model="confirmDeleteTenantModal">
        <x-slot name="title">
            @lang('modules.tenant.deleteTenant')
        </x-slot>
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmDeleteTenantModal')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>

            @if ($seletedTenant)
            <x-danger-button class="ml-3" wire:click='deleteTenant({{ $seletedTenant->id }})' wire:loading.attr="disabled">
                {{ __('Delete') }}
            </x-danger-button>
            @endif
         </x-slot>
    ...
    <x-confirmation-modal wire:model="confirmSelectedDeleteTenantModal">
        <x-slot name="title">
            @lang('modules.tenant.deleteTenant')
        </x-slot>
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmSelectedDeleteTenantModal')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>
