<div>
    ...
    <div class="grid grid-cols-2 mb-6">
        ...
        <div>
            <x-label value="{{ __('modules.settings.billType') }}" />
            <p class="mt-2 text-gray-600 dark:text-gray-200">{{$billTypeId}}</p>
        </div>
    ...
    <div class="grid grid-cols-2 mb-6">
        ...
        <div>
            <x-label class="mb-2" value="{{ __('modules.utilityBills.billDate') }}" />
            <p class="mt-2 text-gray-600 dark:text-gray-200">{{ \Carbon\Carbon::parse($billDate)->format('d F Y') }}</p>
        </div>

        <div>
            <x-label value="{{ __('modules.utilityBills.billAmount') }}" />
            <p class="mt-2 text-gray-600 dark:text-gray-200">{{ currency_format($billAmount) }}</p>
        </div>
    ...
    <div class="grid grid-cols-2 mb-6">
        <div>
            <x-label class="mb-2" value="{{ __('modules.utilityBills.billDueDate') }}" />
            <p class="mt-2 text-gray-600 dark:text-gray-200">{{ \Carbon\Carbon::parse($billDueDate)->format('d F Y') }}</p>

        </div>

        @if($status == 'paid')
            <div>
                <x-label class="mb-2" value="{{ __('modules.utilityBills.billPaymentDate') }}" />
                <p class="mt-2 text-gray-600 dark:text-gray-200">{{ \Carbon\Carbon::parse($billPaymentDate)->format('d F Y') }}</p>
            ...
            <div class="relative w-full group">
                ...
                <div class="relative w-64 h-56 p-1 overflow-hidden rounded bg-gray-50 ring-gray-300 ring-1 dark:ring-gray-500">
                    ...
                    <div class="absolute inset-0 flex items-end justify-end p-2 transition-opacity duration-300 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100">
                        <a href="{{ $billProof }}" target="_blank" class="px-4 py-2 mr-2 text-white rounded shadow hover:bg-gray-800">
                        ...
                        <a href="{{ $billProof }}" download target="_blank" wire:loading.attr="disabled" class="px-4 py-2 text-white rounded shadow hover:bg-gray-800">
    ...
    <div class="grid grid-cols-2 mb-6">
        <a class="min-h-[40px] w-24 rounded-xl bg-white hover:bg-gray-50 text-gray-700 border p-2 inline-flex items-center justify-center gap-1 transition-colors dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700"
            href="{{ route('commonAreaBill.print', $billAmountId) }}" target="_blank">
    ...
    <div class="flex w-full pb-4 mt-6 space-x-4">
        <x-button-cancel  wire:click="$dispatch('hideCommonAreaBillModal')" wire:loading.attr="disabled">@lang('app.cancel')</x-button-cancel>
    </div>
</div>
