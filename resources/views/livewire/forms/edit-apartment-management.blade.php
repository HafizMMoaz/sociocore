<div>
    <form wire:submit="submitForm">
        ...
        <div class="space-y-4">
            ...
            <div class="mb-4">
                <x-label for="floor" :value="__('modules.settings.selectFloor')" required="true" />
                    <x-select id="floor" class="block w-full mt-1" wire:model="selectedFloor">
            ...
            <div>
                ...
                <div x-data="{ isOpen: @entangle('isOpen'), selectedParkingCodes: @entangle('selectedParkingCodes') }" @click.away="isOpen = false">
                    ...
                    <div class="flex items-center space-x-2">
                            ...
                            <ul x-show="isOpen" x-transition class="absolute z-10 w-full mt-1 overflow-auto bg-white rounded-lg shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:focus:border-gray-600 dark:focus:ring-gray-600">
                                ...
                                @empty
                                    <li class="relative py-2 pl-3 text-gray-500 cursor-default select-none pr-9 dark:text-gray-400">
                                        @lang('messages.noParkingManagementFound')
                                    </li>
                                @endforelse
                            </ul>
                        </div>
                        ...
                        <!-- Add Parking Button -->
                        <x-button type="button" class="mt-0" wire:click="$set('showAddParkingModal', true)">
                            @lang('app.add')
                        </x-button>
                    </div>
            ...
            <div>
                ...
                <x-select id="status" class="block w-full mt-1" wire:model.live="status">
                    ...
                    <option value="rented">{{ __('modules.settings.rented') }}</option>
                </x-select>
            </div>
        ...
        <div class="flex w-full pb-4 mt-6 space-x-4">
            ...
            <x-button-cancel  wire:click="$dispatch('hideEditApartmentManagement')" wire:loading.attr="disabled">@lang('app.cancel')</x-button-cancel>
        </div>
    </form>

    <x-dialog-modal wire:model.live="showAddParkingModal">
        <x-slot name="title">
