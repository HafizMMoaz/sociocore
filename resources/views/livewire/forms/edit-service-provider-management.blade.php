<div>
    <form wire:submit="submitForm">
        ...
        <div class="space-y-4">
            <div>
                <x-label for="serviceSettingsId" :value="__('modules.serviceManagement.serviceType')" required="true" />
                <x-select id="serviceSettingsId" class="block w-full mt-1" wire:model="serviceSettingsId">
                    <option value="">@lang('modules.serviceManagement.selectServiceType')</option>
                    @foreach ($serviceSettings as $item)
                        <option value="{{ $item->id }}">{{ $item->name}}</option>
                    @endforeach
                </x-select>
                <x-input-error for="serviceSettingsId" class="mt-2" />
            </div>
        ...
        <div x-data="{ photoName: null, photoPreview: null, hasNewPhoto: @entangle('hasNewPhoto').live , clearFileInput() { this.photoName = ''; this.photoPreview = ''; this.hasNewPhoto = false; this.$refs.photo.value = ''; @this.set('teamLogo', ''); } }" class="col-span-6 sm:col-span-4 mt-2">
