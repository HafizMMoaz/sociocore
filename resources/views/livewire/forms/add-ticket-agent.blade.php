<div>
    <form wire:submit="submitForm">
        ...
        <div class="space-y-4">
            <div>
                <x-label for="selectedAgent" :value="__('modules.settings.societyTicketAgent')"/>
                <x-select id="selectedAgent" class="block w-full mt-1" wire:model="selectedAgent">
                    <option value="">@lang('modules.settings.selectTicketAgent')</option>
                    @foreach ($users as $item)
                        <option value="{{ $item->id }}">{{ $item->name}}</option>
                    @endforeach
                </x-select>
                <x-input-error for="selectedAgent" class="mt-2" />
            </div>
            <div>
