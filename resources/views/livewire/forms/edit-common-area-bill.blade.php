    ...
    <form wire:submit="submitForm">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <x-datepicker class="block w-full mt-1" wire:model.live="billDate" id="billDate" autocomplete="off" placeholder="{{ __('modules.utilityBills.chooseBillDate') }}" :disabled="($status == 'paid')" />
            ...
            {{-- <div x-data="{ photoName: null, photoPreview: null, hasNewPhoto: @entangle('hasNewPhoto').live , clearFileInput() { this.photoName = ''; this.photoPreview = ''; this.hasNewPhoto = false; this.$refs.photo.value = ''; @this.set('teamLogo', ''); } }" class="col-span-6 sm:col-span-4">
                ...
                @if ($commonAreaBill->bill_proof)
                <div class="mt-2" x-show="! photoPreview">
                    <img src="{{ $commonAreaBill->bill_proof_url }}" alt="{{ 'bill Proof' }}" class="object-cover w-20 h-20 overflow-hidden rounded-full">
                </div>
                @endif
            ...
            <div x-data="{
                ...
                @if ($commonAreaBill->bill_proof)
                    <div class="mt-2" x-show="!photoPreview">
                        @if (pathinfo($commonAreaBill->bill_proof_url, PATHINFO_EXTENSION) === 'pdf')
                            <span class="block text-sm text-gray-500 mt-1">{{ basename($commonAreaBill->bill_proof_url) }}</span>
                        @else
                            <img src="{{ $commonAreaBill->bill_proof_url }}" alt="{{ __('Bill Proof') }}" class="object-cover w-20 h-20 overflow-hidden rounded-full">
                        @endif
                    </div>
                @endif
            ...
            @if($status == "paid")
                <div x-data="{
                    ...
                    <!-- Show PDF name or image preview for saved files -->
                    @if ($commonAreaBill->payment_proof)
                        <div class="mt-2" x-show="!photoPreview">
                            @if (pathinfo($commonAreaBill->payment_proof_url, PATHINFO_EXTENSION) === 'pdf')
                                <span class="block text-sm text-gray-500 mt-1">{{ basename($commonAreaBill->payment_proof_url) }}</span>
                            @else
                                <img src="{{ $commonAreaBill->payment_proof_url }}" alt="{{ __('Payment Proof') }}" class="object-cover w-20 h-20 overflow-hidden rounded-full">
                            @endif
                        </div>
                    @endif
        ...
        <div class="flex w-full pb-4 mt-6 space-x-4">
            ...
            <x-button-cancel  wire:click="$dispatch('hideEditCommonAreaBill')" wire:loading.attr="disabled">@lang('app.cancel')</x-button-cancel>
        </div>
    </form>
</div>
