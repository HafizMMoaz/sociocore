    ...
    <form wire:submit.prevent="submitForm">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <x-select class="mt-1 block w-full" wire:model.live='apartment_id' :disabled="($rent->status == 'paid')">
            ...
            <div>
                ...
                <x-select class="mt-1 block w-full" wire:model.live="rent_for_year" id="rent_for_year" :disabled="($rent->status == 'paid')">
                    @for ($year = now()->year; $year >= now()->year - 5; $year--)
                        <option value="{{ $year }}">{{ $year }}</option>
                    @endfor
                </x-select>
                <x-input-error for="rent_for_year" class="mt-2" />
            </div>

            @if($billing_cycle !== 'annually')
                <div>
                    <x-label for="rent_for_month" value="{{ __('modules.rent.rentForMonth') }}" required />
                    <x-select class="mt-1 block w-full" wire:model.live="rent_for_month" id="rent_for_month">
                        @foreach($months as $month)
                            <option value="{{ $month }}" @if($rent_for_month === $month) selected @endif>
            ...
            <div>
                ...
                <x-input id="rent_amount" :disabled="($rent->status == 'paid')" class="block mt-1 w-full" type="number" step="0.01" wire:model='rent_amount' autocomplete="off" />
                <x-input-error for="rent_amount" class="mt-2" />
            </div>

            <div>
                <x-label for="status" value="{{ __('modules.settings.status') }}" required/>
                <x-select id="status" class="block w-full mt-1" wire:model.live="status" :disabled="($rent->status == 'paid')">
                    <option  @if($rent->status == 'paid') selected @endif   value="paid">{{ __('modules.rent.paid') }}</option>
                    <option  @if($rent->status == 'unpaid') selected @endif   value="unpaid">{{ __('modules.rent.unpaid') }}</option>
                </x-select>
            ...
            @if ($status === 'paid')
                ...
                <div x-data="{ photoName: null, photoPreview: @if($rent->payment_proof) '{{ url('user-uploads/' . $rent->payment_proof) }}' @else null @endif }" x-on:photo-removed.window="photoName = null; photoPreview = null;">
            ...
            <div class="flex w-full pb-4 space-x-4 mt-6">
                ...
                <x-button-cancel wire:click="$dispatch('hideEditRent')" wire:loading.attr="disabled">@lang('app.cancel')</x-button-cancel>
            </div>
        </div>
    </form>
</div>
