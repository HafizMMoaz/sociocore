    ...
    <form wire:submit="submitForm">
        ...
        <div class="space-y-4">
            ...
            @if($selectedApartment)
                <div class="p-4 bg-gray-40 border-2 border-gray-500 rounded-lg">
                    @if ($selectedApartment->status === 'rented' && $selectedApartment->tenants->isNotEmpty())
                        <p class="block text-sm font-medium">
                            @lang('modules.visitorManagement.tenantName')
                            {{ $selectedApartment->tenants->first()->user->name}}
                        </p>
                        <p class="block text-sm font-medium">
                            @lang('modules.visitorManagement.tenantNumber')
                            {{ $selectedApartment->tenants->first()->user->phone_number }}
                        </p>
                    @else
                        <p class="block text-sm font-medium">
            ...
            <div class="grid grid-cols-2 gap-x-4">
                <div>
                    <x-label for="visitorName" value="{{ __('modules.visitorManagement.visitorName') }}" required="true" />
            ...
            <div>
                <x-label for="visitorAddress" value="{{ __('modules.visitorManagement.visitorAddress') }}" required="true" />
            ...
            <div class="grid grid-cols-2 gap-x-4">
                ...
                <div>
                    <x-label for="dateOfVisit" value="{{ __('modules.visitorManagement.dateOfVisit') }}" required="true"/>
            ...
            <div class="grid grid-cols-2 gap-x-4">
                ...
                <div>
                    ...
                    <div class="relative mt-1">
                        <div class="absolute inset-y-0 end-0 top-1 flex items-center pe-3.5 pointer-events-none">
                            <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                <path fill-rule="evenodd" d="M2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10S2 17.523 2 12Zm11-4a1 1 0 1 0-2 0v4a1 1 0 0 0 .293.707l3 3a1 1 0 0 0 1.414-1.414L13 11.586V8Z" clip-rule="evenodd"/>
                            </svg>
                        </div>
        ...
        <div class="flex w-full pb-4 mt-6 space-x-4">
            ...
            <x-button-cancel  wire:click="$dispatch('hideAddVisitor')" wire:loading.attr="disabled">@lang('app.cancel')</x-button-cancel>
        </div>
    </form>
</div>
