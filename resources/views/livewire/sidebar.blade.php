            ...
            <div class="flex flex-col flex-1 pt-5 pb-4 overflow-y-auto mb-16 [&::-webkit-scrollbar]:w-1.5
                ...
                <div
                    ...
                    @if (
                        ...
                        <x-sidebar-dropdown-menu :name='__("menu.bills")' icon='utility_bills_management' :active='request()->routeIs(["utilityBills.*" , "common-area-bill.*" , "maintenance.*"])'>
                        ...
                        <x-sidebar-dropdown-menu :name='__("menu.assetsManagement")' icon='assets' :active='request()->routeIs(["assets.*", "asset-maintenance.*", "asset-issue.*"])'>
                            @if(user_can('Show Assets') || isRole() == 'Owner' || isRole() == 'Tenant')
                                @if($this->hasModule('Assets'))
                                    @livewire('sidebar-dropdown-menu', ['name' => __('menu.assetsManagement'), 'link' => route('assets.index'), 'active' => request()->routeIs('assets.index')])
                    ...
                    @if (user_can('Show Maintenance'))
                        @if($this->hasModule('Maintenance') && $this->hasSocietyModule('Maintenance'))
                            <x-sidebar-dropdown-menu :name='__("menu.report")' icon='reports' :active='request()->routeIs(["reports.maintenance"])'>
                                @if(user_can('Show Maintenance'))
                                    @if($this->hasModule('Maintenance') && $this->hasSocietyModule('Maintenance'))
