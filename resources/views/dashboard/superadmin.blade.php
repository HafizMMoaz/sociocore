...
<div class="grid grid-cols-1">
    ...
    <div class="p-4">
        <h1 class="text-xl font-semibold text-gray-900 sm:text-xl dark:text-white my-2 ">@lang('modules.dashboard.statistics')</h1>

        <div class="grid w-full grid-cols-1 gap-4 xl:grid-cols-4 py-4">

            @livewire('dashboard.societyCount')

            @livewire('dashboard.totalSocietyCount')

            @livewire('dashboard.totalFreeSocietyCount')

            @livewire('dashboard.totalPaidSocietyCount')

        </div>

        <div class="grid w-full grid-cols-1 gap-4 xl:grid-cols-2">

            @livewire('dashboard.subscriptionChart')

            @livewire('dashboard.onlineOfflinePayment')

            @livewire('dashboard.pendingOfflineRequest')

            @livewire('dashboard.latestBilling')
