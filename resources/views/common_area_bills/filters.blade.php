...
<div class="w-full p-4 flex flex-wrap gap-2 sm:gap-4  bg-gray-50 dark:bg-gray-900">
        ...
        <x-dropdown align="left" dropdownClasses="z-10">
            <x-slot name="trigger">
                <span class="inline-flex rounded-md">
                    <button type="button"
                        ...
                        @lang('modules.settings.filterBillTypes')
                        <div
                            class="inline-flex items-center justify-center w-5 h-5 text-xs font-medium text-white bg-red-500 rounded-md dark:border-gray-900 ml-1 {{ count($filterBillTypes) == 0 ? 'hidden' : '' }}">
                            {{ count($filterBillTypes) }}
                        </div>
            ...
            <x-slot name="content">
                ...
                @foreach ($billTypes as $billType)
                    <x-dropdown-link wire:key="billType-option-{{ $billType->id }}" onclick="event.stopPropagation()">
                        <input id="apartment-{{ $billType->id }}" type="checkbox"
                            value="{{ $billType->name }}" wire:model.live="filterBillTypes"
                        ...
                        <label for="apartment-{{ $billType->id }}"
    ...
    <div>
        <x-dropdown align="left" dropdownClasses="z-10">
            ...
            <x-slot name="content">
                ...
                @foreach (['unpaid' => __('modules.utilityBills.unpaid'), 'paid' => __('modules.utilityBills.paid')] as $key => $label)
    ...
    <div class="flex  gap-2 x-4">
        <div>
            <!-- Start Date -->
            <label for="start-date" class="text-sm font-medium text-gray-900 dark:text-gray-100">
                @lang('modules.visitorManagement.startDate')
            </label>
            <x-datepicker wire:model.live="startDate" id="start-date" placeholder="YYYY-MM-DD" />
        </div>

        <div>
            <!-- End Date -->
            <label for="end-date" class="text-sm font-medium text-gray-900 dark:text-gray-100">
                @lang('modules.visitorManagement.endDate')
            </label>
            <x-datepicker wire:model.live="endDate" id="end-date" placeholder="YYYY-MM-DD" />
        </div>

        <!-- Clear Filters Button -->
        @if ($clearFilterButton)
            <div wire:key='filter-btn-{{ microtime() }}' class="mt-6">
                <x-danger-button wire:click='clearFilters'>
                    <svg aria-hidden="true" class="w-5 -ml-1 h-7 sm:mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"
        ...
        @endif
            <!-- Hide Filters Button -->
        {{-- <div> --}}
            <x-secondary-button wire:click="$toggle('showFilters')" class="mt-6">@lang('app.hideFilter')</x-secondary-button>
        {{-- </div> --}}
    </div>
</div>
